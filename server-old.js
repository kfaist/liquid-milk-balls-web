...
const INPUT_ROOM = process.env.LIVEKIT_ROOM_NAME || 'mirrors-echo-input';
const PROCESSED_ROOM = process.env.LIVEKIT_PROCESSED_ROOM || 'mirrors-echo-processed';
...
// ============================================================
// WEBSOCKET SIGNALING (WebRTC SDP/ICE relay)
// ============================================================
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  if (LIVEKIT_API_KEY && LIVEKIT_API_SECRET && LIVEKIT_URL) {
    ...
    console.log(`\n📊 API Endpoints:`);
    console.log(`   GET  /api/viewer-token - View input (for OBS)`);
    console.log(`   GET  /api/processed-publisher-token - OBS WHIP publishing`);
    console.log(`   GET  /api/processed-viewer-token - View processed output`);
    console.log(`   POST /api/token - Generic token endpoint`);
    console.log(`   POST /api/create-rtmp-ingress - Create RTMP stream`);
    console.log(`   GET  /api/list-ingresses - List RTMP ingresses`);
  } else {
  ...
  if (process.env.STRIPE_SECRET_KEY) {
    ...
    console.log(`   POST /api/create-checkout-session - Create checkout`);
  } else {
    console.log(`\n⚠️  Stripe not configured. Set STRIPE_SECRET_KEY environment variable.`);
  }
});
